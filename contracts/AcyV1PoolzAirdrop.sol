...
contract Invest is PoolsData {
    ...
    function NewInvestor(
        ...
        poolInvestor[_PoolId].IndexToAddress[TotalInvestors] = _Sender;
...
contract AcyV1PoolzAirdrop is InvestorData {
    ...
    function WithdrawERC20ByAirdrop(uint _PoolId, uint _startIndex, uint _range) isPoolId(_PoolId) onlyOwnerOrGov public {
        require(
            GetPoolStatus(_PoolId) == PoolStatus.Distributing,
            "could not withdraw ERC20 now"
        );
        require(
            _startIndex + _range < poolInvestor[_PoolId].TotalInvestors,
            "index error"
        );
        for (uint index = _startIndex; index < _startIndex + _range; index ++) {
            uint TotalTokensAmount = poolInvestor[_PoolId].Investors[
                    poolInvestor[_PoolId].IndexToAddress[index]
                ].TotalTokensAmount;
            uint DistributedAmount = poolInvestor[_PoolId].Investors[
                    poolInvestor[_PoolId].IndexToAddress[index]
                ].DistributedAmount;
            uint totalShare = 0;
            uint availableShare = 0;
